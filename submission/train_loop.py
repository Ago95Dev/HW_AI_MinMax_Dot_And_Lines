...
from benchmarks import GreedyAgent, ClassicalMinMaxAgent
import time
...
class TrainingLoop:
    ...
    def __init__(self, grid_size: int = 3, hidden_sizes: List[int] = [128, 64]):
        ...
        self.benchmark_results = [] # Track benchmark performance
        
        # Fixed set of states for signal stability monitoring
        self.monitor_states = self._generate_monitor_states(count=20)
        self.signal_stability = []

    def _generate_monitor_states(self, count: int) -> List[np.ndarray]:
        """Generate a fixed set of random states to monitor prediction stability."""
        states = []
        for _ in range(count):
            game = DotsAndBoxes(grid_size=self.grid_size)
            # Make random moves to reach a mid-game state
            num_moves = np.random.randint(1, self.grid_size * (self.grid_size + 1) * 2)
            for _ in range(num_moves):
                if game.is_game_over(): break
                moves = game.get_valid_moves()
                if not moves: break
                game.make_move(moves[np.random.randint(0, len(moves))])
            states.append(game.get_state_vector())
        return states
    ...
    def train_iteration(self, L: int, K: int, num_games: int = 10, 
        ...
        stability = self.mlp.get_prediction_variance(self.monitor_states)
        self.signal_stability.append(stability)
        
        # Run benchmarks periodically
        benchmark_stats = {}
        if (len(self.training_history) + 1) % 5 == 0: # Every 5 iterations
             benchmark_stats = self.evaluate_against_benchmarks(L, K)
             self.benchmark_results.append(benchmark_stats)
        ...
        if verbose:
            ...
            print(f"Signal Stability (Variance): {stability:.4f}")
            if benchmark_stats:
                print("\nBenchmark Results (Win Rate):")
                for agent, res in benchmark_stats.items():
                    print(f"  vs {agent}: {res['win_rate']:.2f}")
            print(f"Time: {elapsed_time:.2f}s")
        
        return stats

    def evaluate_against_benchmarks(self, L: int, K: int, num_games: int = 10) -> dict:
        """
            ...
            for i in range(num_games):
                ...
                while not game.is_game_over():
                    if game.current_player == 1:
                        move = p1.select_move(game)
                    else:
                        move = p2.select_move(game)
                    game.make_move(move)
                
                winner = game.get_winner()
                if winner == 0:
                    ties += 1
                elif winner == agent_idx:
                    wins += 1
                else:
                    losses += 1
